...
export const instaSW = async function waStoryInsta(waNumber, shortcode) {
  ...
  const waStoryDoc = new GoogleSpreadsheet(process.env.waStoryID, googleAuth); //Google Authentication for client DB    
  ...
  if (shortcode !== null) {
    ...
    let clientResponse = await _sheetDoc(process.env.clientDataID, 'ClientData');
    ...
    if (hasSheetName) {
      ...
      let userResponse = await _sheetDoc(process.env.userDataID, clientName);
