                    ...
                    for (let i = 0; i < response.length; i++){
                        ...
                        if (response[i].get('STATUS') === "TRUE" && response[i].get('INSTA_STATE') === "TRUE" && response[i].get('TYPE') === ciceroKey.ciceroClientType) {         
                            ...
                            switch (loadInsta.code) {
                                case 200:
                                    console.log(time+" "+response[i].get('CLIENT_ID')+' SUCCESS LOAD INSTA DATA');
                                    await client.sendMessage('6281235114745@c.us', response[i].get('CLIENT_ID')+' SUCCESS LOAD INSTA DATA');                        
                                    reportInsta = await reportInstaLikes(response[i]);
                                    sendResponse('6281235114745@c.us', reportInsta, response[i].get('CLIENT_ID')+' ERROR LOAD INSTA DATA');
                                    break;                                           
                                case 303:
                                             ...
                                             await getInstaPost(data[i]).then(
                                                            ...
                                                            await newReportInsta(response[i]).then(
                                                                ...
                                                                async data => {
                                                                    switch (data.code) {
                                                                        case 303:
                                                                            console.log(data.data);
                                                                            await client.sendMessage('6281235114745@c.us', data[i].get('CLIENT_ID')+' ERROR REPORT INSTA POST');
                                                                            break;
                                                                        default:
                                                                            await client.sendMessage('6281235114745@c.us', data[i].get('CLIENT_ID')+' '+data.data);
                                                                            break;
                                                                    }
                                                            });
                                                        }
                                                ...
                                                async data => {
                                                    switch (data.code) {
                                                        case 303:
                                                            console.log(data.data);
                                                            await client.sendMessage('6281235114745@c.us', data[i].get('CLIENT_ID')+' ERROR GET INSTA POST');
                                                            break;
                                                        default:
                                                            await client.sendMessage('6281235114745@c.us', data[i].get('CLIENT_ID')+' '+data.data);
                                                            break;
